                ...
                @forelse($projects as $index => $project)
                    <div class="{{ $index === 0 ? 'md:col-span-2' : '' }} bg-white rounded-2xl overflow-hidden shadow-sm border border-gray-100 card-hover group cursor-pointer"
                        ...
                        <div class="{{ $index === 0 ? 'h-52' : 'h-40' }} overflow-hidden relative">
                            <img src="{{ $project->image ? asset($project->image) : 'https://picsum.photos/seed/proj'.($index+1).'/700/300' }}" alt="{{ $project->title }}"
                                class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                            @if($index === 0)
                            <div class="absolute top-3 left-3 bg-primary text-white text-xs font-semibold px-3 py-1 rounded-full">
                                Featured</div>
                            @endif
                        </div>
                        <div class="p-5 {{ $index === 0 ? 'md:p-6' : '' }}">
                            ...
                            <h3 class="text-{{ $index === 0 ? 'lg' : 'base' }} font-bold text-dark mb-2">{{ $project->title }}</h3>
                            <p class="text-sm text-gray-500 leading-relaxed mb-{{ $index === 0 ? '4' : '3' }}">{{ $project->description }}</p>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div class="bg-white/10 backdrop-blur rounded-2xl p-6 border border-white/20">
                    <div class="flex gap-1 mb-3 text-yellow-400">
                        {{ str_repeat('★', $testimonial->rating) }}<span class="text-white/30">{{ str_repeat('★', 5 - $testimonial->rating) }}</span>
                    </div>
                    ...
                    <div class="flex items-center gap-3">
                        <img src="{{ $testimonial->avatar ? asset($testimonial->avatar) : 'https://api.dicebear.com/9.x/adventurer/svg?seed=' . urlencode(strtolower($testimonial->client_name)) . '&backgroundColor=ffd5dc' }}"
                            alt="{{ $testimonial->client_name }}" class="w-10 h-10 rounded-full border-2 border-indigo-300 object-cover bg-white" />
                        <div>
            ...
            <div class="bg-gray-50 rounded-2xl p-8 border border-gray-100">
                ...
                @if($messageSent)
                    <div class="flex flex-col items-center justify-center py-10 text-center">
                        ...
                        <p class="text-gray-500 text-sm mb-6">Thanks for reaching out. I'll get back to you within 24 hours.</p>
                ...
                <div class="space-y-4">
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="text-sm font-medium text-gray-600 mb-1 block">First Name <span class="text-red-400">*</span></label>
                            ...
                            @error('contact.first_name') <span class="text-xs text-red-500 mt-0.5 block">{{ $message }}</span> @enderror
                        </div>
                        <div>
                    ...
                    <div>
                        <label class="text-sm font-medium text-gray-600 mb-1 block">Email <span class="text-red-400">*</span></label>
                        <input wire:model="contact.email" type="email" placeholder="john@example.com"
                            class="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-primary focus:border-transparent transition-all" />
                        @error('contact.email') <span class="text-xs text-red-500 mt-0.5 block">{{ $message }}</span> @enderror
                    </div>
                    <div>
                    ...
                    <div>
                        <label class="text-sm font-medium text-gray-600 mb-1 block">Message <span class="text-red-400">*</span></label>
                        ...
                        @error('contact.message') <span class="text-xs text-red-500 mt-0.5 block">{{ $message }}</span> @enderror
                    </div>
